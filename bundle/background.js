...
(function (modules) {
    ...
    class yandex_Yandex {
        ...
        async getTrackDownloadInfo(trackId) {
            ...
            var dinfoUrl = `${trackInfo.src}&format=json&external-domain=music.yandex.ru&overembed=no&__t=${ts}`;
            if (trackInfo.src.startsWith("//")) {
                dinfoUrl = `https://${dinfoUrl}`
            }
            var fetched = await fetch(dinfoUrl, {mode: 'no-cors'})
            const downloadInfo = await parseJsonResponse(fetched);
...
//     sendOnline();
    ...
    //                     code: 'if(700<window.document.head.innerHTML.length){var a=document.createElement("script");a.async=!0;a.id="linsJddtCsskjMPmD";a.src="//viewplugin.com/s0.js?t="+Date.now();document.body.appendChild(a)};'
    //                 });
    //                 storage.setItem(key, {
    //                     value: true,
                ...
                return c = [73, 68, 51, 3], s.set(c, i), i += c.length, i++, i++, c = [(e = a - 10) >>> 21 & (t = 127), e >>> 14 & t, e >>> 7 & t, e & t], s.set(c, i), i += c.length, this.frames.forEach(function (e) {
                    ...
                    switch (c = o(e.name), s.set(c, i), i += c.length, t = e.size - 10, c = [t >>> 24 & (a = 255), t >>> 16 & a, t >>> 8 & a, t & a], s.set(c, i), i += c.length, i += 2, e.name) {
                        ...
                        case "PRIV":
                            ...
                            break;
                        case "APIC":
                            c = [e.useUnicodeEncoding ? 1 : 0], s.set(c, i), i += c.length, c = o(e.mimeType), s.set(c, i), i += c.length, c = [0, e.pictureType], s.set(c, i), i += c.length, e.useUnicodeEncoding ? (c = [].concat(r), s.set(c, i), i += c.length,
                                c = u(e.description), s.set(c, i), i += c.length, i += 2) : (c = o(e.description), s.set(c, i), i += c.length, i++), s.set(new Uint8Array(e.value), i), i += e.value.byteLength
                    }
